...
def main():
    ...
    button_selecionar = tk.Button(root, text="1º Selecionar arquivo", command=selecionar_arquivo)
    ...
    button_exportar = tk.Button(root, text="2º Selecionar a pasta para exportar", command=exportar)
    button_exportar.grid(row=2, column=0, sticky='W' + 'E')
    # Cria um label para mostrar o caminho do exportar
    label_exportar = tk.Label(root, text="Escolha a pasta para exportar o arquivo, pode ser qualquer uma")
    label_exportar.grid(row=3, column=0, sticky='W')


    # Cria um botão para rodar o script
    button_run = tk.Button(root, text="Tirar silêncio e exportar para Adobe Premiere ou Sony Vegas", command=rodar)
    ...
    button_run = tk.Button(root, text="Tirar silêncio e exportar para DaVinci Resolve", command=vinci)
...
def rodar():
    ...
    subprocess.run(["auto-editor", caminho_arquivo, "--export", "premiere", "--margin", "0.1sec"], shell=False)
    messagebox.showinfo("Parabéns", "Exportação para o Premiere ou Sony Vegas concluída")
